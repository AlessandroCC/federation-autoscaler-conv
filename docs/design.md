...
| **ConsumerChoice** | Delegated to a local LLM | The Broker returns **all** available providers with no masking; the Consumer Agent passes the full list plus the user's natural-language request (`ConsumerPolicySpec.UserPrompt`) to a local LLM (e.g. Ollama), which interprets it and selects a single provider. Falls back deterministically (first provider with capacity) if the AI is unreachable or returns an invalid choice. |
