...
# ============================================================
#  Usage:
#    bash tests/scalability/run-scalability-test.sh [--config FILE] [--keep-cluster]
#
#  Configuration:
#    tests/configs/scalability.yaml sets the only two things that change
...
#
#    --config FILE    use another YAML file (default tests/configs/scalability.yaml)
...
#  Arguments
# -----------------------------------------------------------
CONFIG_FILE="$REPO_ROOT/tests/configs/scalability.yaml"
KEEP_CLUSTER=false
while [[ $# -gt 0 ]]; do
  case "$1" in
    --config)
