...
Direct-comparison variant of ecoDiagramMaker.py / ecoDiagramMaker2.py (see
those files for the full background on the underlying aggregation logic,
which this script reuses unchanged).

Where ecoDiagramMaker.py / ecoDiagramMaker2.py plot Phase A and Phase B
sequentially on one shared timeline (with or without the dead time between
them removed), this script instead overlays them: Phase A (Random) and
Phase B (Eco) are each measured from THEIR OWN start (elapsed minutes since
that phase's first valid event), then plotted on the SAME X axis, so a
viewer can compare "N minutes into the phase" directly between the two
policies -- e.g. "at minute 5, was the federation greener under Random or
under Eco?" -- instead of seeing them as two segments of one longer line.

There is no "gap" concept here (each curve starts at its own X=0), so unlike
ecoDiagramMaker2.py there is nothing to remove or anchor.
...
experiment size, e.g.:
    ...
    python3 tests/scripts/ecoDiagramMaker3.py --input results/3c-7p/reservations.csv
    python3 tests/scripts/ecoDiagramMaker3.py --input results/8c-17p/reservations.csv
    python3 tests/scripts/ecoDiagramMaker3.py --input results/15c-35p/reservations.csv
    python3 tests/scripts/ecoDiagramMaker3.py --input results/30c-70p/reservations.csv
...
def write_summary(
    ...
    *,
    ...
    lines = [
        ...
        "**This is the overlaid-comparison variant**: Phase A and Phase B are each "
        "measured from their own start and plotted on the same X axis -- see "
        "`ecoDiagramMaker.py` / `ecoDiagramMaker2.py` for the sequential-timeline "
        ...
        *warnings_lines,
        "\n## Reproduce\n\n",
        "```bash\n",
        f"python3 tests/scripts/ecoDiagramMaker3.py --input {input_path}\n",
        "```\n",
    ]
