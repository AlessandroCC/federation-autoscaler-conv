...
the same command works unchanged for every experiment size, e.g.:
    ...
    python3 tests/scripts/latencyDiagramMaker.py --input results/3c-7p/reservations.csv
    python3 tests/scripts/latencyDiagramMaker.py --input results/30c-70p/reservations.csv
...
def load_reservations(path: Path) -> pd.DataFrame:
    ...
    if missing:
        sys.exit(
            ...
            "(see tests/testlib/writer.go, ReservationRecord)."
        )
    ...
    *,
    ...
    lines = [
        ...
        *warnings_lines,
        "\n## Reproduce\n\n",
        "```bash\n",
        f"python3 tests/scripts/latencyDiagramMaker.py --input {input_path}\n",
        "```\n",
    ]
...
def main(argv: list[str] | None = None) -> None:
    ...
    if is_eco_csv(df):
        sys.exit(
            ...
            "comparative-eco reservations.csv. Use tests/scripts/ecoDiagramMaker.py for it."
        )
